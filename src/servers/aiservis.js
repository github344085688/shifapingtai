...
class AIService {
    constructor(aiConfig) {
        ...
        Object.defineProperty(this, "mockService", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "originalSendToAI", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
    ...
    // 辅助方法：创建带样式的消息
    createStyledMessage(content, style) {
        return TextProcessor.createStyledMessage(content, style);
    }
    // 辅助方法：处理带头部的消息
    handleMessageWithHeader(content, hasShownHeader, headerText, callback, isThinking = true) {
        const style = isThinking ? STYLES.THINKING_HEADER : STYLES.ANSWER_HEADER;
        return TextProcessor.handleMessageWithHeader(content, hasShownHeader, headerText, callback, isThinking, style);
    }
    async sendToAI(message, callback, lastMessage = null) {
        // console.log('import.meta.env.DEV2222222222', this.aiConfig)
        // 在发送到 AI 之前先记录次数，确保 POST 成功后再继续
        if (this.aiConfig.isTimer)
            await setTimes();
        let systemMessages = [
            {
                role: 'system',
        ...
        try {
            ...
            const Intercepted = await interceptData(response);
    ...
    async responseReader(response, abortController, callback, hasShownThinkingHeader, hasShownAnswerHeader, lastMessage, hasShownSearchProcessHeader) {
                ...
                for (const chunk of chunks) {
                    ...
                    try {
                        ...
                        if (isGptModel || isLegalInterface) {
                            ...
                            const hasDeltaContent = typeof deltaContentRaw === 'string'
                            ...
                            if (additionalData) {
                                ...
                                if (additionalData.type === 'citation') {
                                    ...
                                    const isBlockedId = (id) => typeof id === 'string' &&
                                ...
                                switch (additionalData.type) {
                                    ...
                                    case 'start_res':
                                        // 处理搜索结果开始
                                        hasShownThinkingHeader = this.handleMessageWithHeader(MESSAGES.SEARCH_START, hasShownThinkingHeader, MESSAGES.THINKING_PROCESS, callback);
                                        break;
                                    case 'get_res':
                                            ...
                                            try {
                                                ...
                                                if (resData.content) {
                                                    ...
                                                    if (!hasShownThinkingHeader) {
                                                        callback(TextProcessor.createStyledMessage(MESSAGES.THINKING_PROCESS, STYLES.THINKING_HEADER) + contentDiv, false, true);
                                                ...
                                                // 如果解析失败，使用原来的逻辑
                                                hasShownThinkingHeader = this.handleMessageWithHeader(`📄 ${additionalData.data}`, hasShownThinkingHeader, MESSAGES.THINKING_PROCESS, callback);
                                            }
                                        }
                                        break;
                                    case 'answer':
                                    ...
                                    case 'queries':
                                        ...
                                        try {
                                            ...
                                            if (Array.isArray(queries) && queries.length > 0) {
                                                hasShownThinkingHeader = this.handleMessageWithHeader(`${MESSAGES.SEARCH_KEYWORDS}${queries.join(', ')}`, hasShownThinkingHeader, MESSAGES.THINKING_PROCESS, callback);
                                            }
                                        }
                                        catch (e) {
                                    ...
                                    case 'law_article':
                                        ...
                                        if (additionalData.data && additionalData.data.trim()) {
                                            try {
                                                ...
                                                const contentDiv = `<div style="${STYLES.THINKING_CONTENT}">${formattedContent}</div>`;
                                                hasShownThinkingHeader = this.handleMessageWithHeader(contentDiv, hasShownThinkingHeader, MESSAGES.THINKING_PROCESS, callback);
                                            }
                                            catch (e) {
                                    ...
                                    default:
                                        ...
                                        if (additionalData.data && additionalData.data.trim()) {
                                            ...
                                            hasShownThinkingHeader = this.handleMessageWithHeader(processedData, hasShownThinkingHeader, MESSAGES.THINKING_PROCESS, callback);
                                        }
                                        break;
                                }
                            }
    ...
    processLegalAnswerContent(content) {
        return content;
    }
    // 修改：处理法律接口的通用内容 - 统一使用 processSearchResultContent
    processLegalContent(content) {
        return content;
    }
    // 测试方法：使用模拟数据进行测试
    async sendToAIMock(message, callback, lastMessage) {
...
//# sourceMappingURL=aiservis.js.map