...
export class TextProcessor {
  ...
  static processHtmlContent(content: string): string {
    ...
    processedContent = processedContent.replace(
      /<br>##\s+(.+?)(?=<br>|$)/g,
      '<br><h2 style="font-size: 1.4em; font-weight: bold; margin: 20px 0 12px 0; color: #333;">$1</h2>',
    )
    processedContent = processedContent.replace(
      /<br>#\s+(.+?)(?=<br>|$)/g,
    ...
    //hsdfhajdshfhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh
    // 1. 压缩多个 <br> 只保留一个
