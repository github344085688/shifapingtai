...
import aiConfig, { administrativeLawEnforcementAssistant, CACHE_DURATION } from '@/config/aiConfig';
...
// 使用动态title功能
const { title, updateTitle } = useTitle('行政执法助手');
defineComponent({
...
onMounted(() => {
    ...
    if (!globalState.administrativeLawEnforcementAssistant) {
        globalState.administrativeLawEnforcementAssistant = {};
    }
    // 页面渲染前判断缓存是否有效
    if (globalState.administrativeLawEnforcementAssistant.aiResults &&
        globalState.administrativeLawEnforcementAssistant.generalAiTime) {
        const currentTime = Date.now();
        const savedTime = globalState.administrativeLawEnforcementAssistant.generalAiTime;
        const timeDifference = currentTime - savedTime;
        // 如果时间差小于常量（12小时），则使用缓存
        if (timeDifference < CACHE_DURATION) {
            messages.value = globalState.administrativeLawEnforcementAssistant.aiResults;
            ...
            messages.value = [];
            delete globalState.administrativeLawEnforcementAssistant.aiResults;
            delete globalState.administrativeLawEnforcementAssistant.generalAiTime;
        }
    }
    if (globalState.administrativeLawEnforcementAssistant.aiResults)
        messages.value = globalState.administrativeLawEnforcementAssistant.aiResults;
...
// console.log(administrativeLawEnforcementAssistant)
// 创建AIService实例时传入配置
const aiConfigs = {
    api: administrativeLawEnforcementAssistant.api,
    apiKey: getApiKeyFromUrl(),
    model: administrativeLawEnforcementAssistant.model,
    isTimer: true,
};
...
const setMessage = (message, isDone, isThinking, isError = false) => {
    ...
    if (isDone) {
        ...
        globalState.administrativeLawEnforcementAssistant.aiResults = messages.value;
        globalState.administrativeLawEnforcementAssistant.generalAiTime = Date.now();
        return;
    }
...
const __VLS_0 = __VLS_asFunctionalComponent(SendMessages, new SendMessages({
    ...
    title: (__VLS_ctx.administrativeLawEnforcementAssistant.title),
    placeholder: (__VLS_ctx.administrativeLawEnforcementAssistant.placeholder),
    note: (__VLS_ctx.administrativeLawEnforcementAssistant.note),
}));
...
const __VLS_self = (await import('vue')).defineComponent({
    setup() {
        return {
            AiText: AiText,
            administrativeLawEnforcementAssistant: administrativeLawEnforcementAssistant,
...
//# sourceMappingURL=administrativeLawEnforcementAssistant.vue.js.map